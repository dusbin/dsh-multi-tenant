...
export function apply(ctx, config) {
  ...
  // 运行时韧性：对端（浏览器/上游/LLM 出站等）随时可能 RST 连接。Node 中任何
  // 流/socket 收到 ECONNRESET/EPIPE 而无人监听 'error' 都会以 unhandled 'error'
  // 事件杀死整个进程（含 DSH 本体/其他插件代码的裸 socket）。这里对**连接类**
  // 错误做兜底：仅记录并继续（连接错误不污染进程状态，业界代理通行做法）；
  // 其余异常照常抛出（保留默认崩溃语义，便于暴露真 bug）。
  ...
  const onUncaughtException = (error) => {
    if (error && error.code && SOCKET_ERROR_CODES.has(error.code)) {
      log.warn(`[dsh-multi-tenant] swallowed connection-level error (${error.code}): ${error.message}`);
      return;
    }
    throw error; // 非连接类：保持默认崩溃
  };
