...
export function apply(ctx, config) {
  ...
  const authService = createAuthService(store, cfg, ldapStrategy);
  ...
  // /mt 管理通道（多租户/RBAC 管理 API，挂 DSH webserver）
  try {
    wireMtRoute(ctx, { store, authService, mtChannel });
    log.info('[dsh-multi-tenant] /mt management channel mounted');
  } catch (error) {
    log.warn(`[dsh-multi-tenant] /mt channel mount failed: ${error.message}`);
  }
