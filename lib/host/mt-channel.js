...
import { exportData, exportSummary } from './backup.js';
...
function deny(error) {
  // details 必填：满足 DSH 客户端 RpcResult 校验（serverResponseSchema）
  ...
  function dispatch(endpoint, payload, auth) {
    ...
    try {
      switch (endpoint) {
        ...
        case 'user.create': {
          if (!hasRole(user, 'admin')) return deny({ code: MT_ERRORS.FORBIDDEN, message: 'admin required' });
          if (user.role === 'system' && (p.tenantId === undefined || p.tenantId === null)) {
            return deny({ code: 'tenant-required', message: 'platform admin must specify tenantId when creating a user' });
          }
          const targetTenant = p.tenantId === undefined || p.tenantId === null ? (user.tenant_id ?? user.tenantId) : Number(p.tenantId);
          if (!canAccessTenant(user, targetTenant)) return deny({ code: MT_ERRORS.FORBIDDEN, message: 'tenant out of scope' });
        ...
        case 'user.setStatus': {
          ...
          if (target.id === user.id) {
            return deny({ code: 'self-operation', message: 'cannot change your own status' });
          }
        ...
        case 'user.revokeSessions': {
          ...
          if (!target) return deny({ code: MT_ERRORS.NOT_FOUND, message: 'user not found' });
          if (target.id === user.id) {
            return deny({ code: 'self-operation', message: 'cannot force logout yourself' });
          }
        ...
        // ---------------- data.export（平台管理员全量备份）----------------
        case 'data.export': {
          if (!isSystem(user)) return deny({ code: MT_ERRORS.FORBIDDEN, message: 'platform admin required' });
          const data = exportData(store);
          store.writeAudit({ actorUserId: user.id, action: 'data.export', result: 'success' });
          return okValue({ export: data, summary: exportSummary(data), note: '导出含密码哈希，请妥善保管' });
        }
