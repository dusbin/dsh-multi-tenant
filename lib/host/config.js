...
export const DEFAULT_CONFIG = {
  ...
  auth: {
    local: {
      ...
      autoLock: true,        // 连续失败达阈值自动锁定账号（管理员解锁）
    },
    bootstrap: {
      enabled: true,
    },
    ...
    oidc: {
      enabled: false,
      issuerUrl: '',         // OIDC 发现端点（如 https://accounts.example.com）
      clientId: '',
      clientSecret: '',
      publicBaseUrl: '',     // 对外基址（如 https://ds.mycompany.com；空 = 取请求 Host）
      redirectPath: '/api/auth/oidc/callback',
      scopes: 'openid profile email',
      claimsMapping: {       // 声明 → 本地字段
        subject: 'sub',
