 ...
 */
export function createAuthService(store, cfg) {
  ...
  /**
   * 邮箱密码登录。
   * @returns {{ok: true, user: object, token: string} | {ok: false, error: {code: string, message: string}}}
   */
  function login({ username, password, ip = null, userAgent = null }) {
    if (!cfg.auth.local.enabled) {
      return { ok: false, error: { code: AUTH_ERRORS.INVALID_CREDENTIALS, message: 'local login is disabled' } };
    }
    const auditDenied = (code, detail = {}) => {
      try {
        const actor = store.getUserByUsername(username);
        store.writeAudit({ actorUserId: actor ? actor.id : null, tenantId: actor ? actor.tenant_id : null, action: 'auth.login', detail: { reason: code, ...detail }, result: 'denied', ip });
      } catch {
        /* audit best-effort */
      }
    };
    ...
    const user = store.getUserByUsername(username) ?? (username.includes('@') ? store.getUserByEmail(username) : undefined);
    ...
    store.writeAudit({ actorUserId: user.id, tenantId: user.tenant_id, action: 'auth.login', result: 'success', ip });
  ...
  function me(cookieHeader) {
    if (bootstrapRequired()) {
      return { ok: true, user: null, bootstrapRequired: true };
    }
    ...
    return { ok: true, user: publicUser(auth.user), bootstrapRequired: false };
  }

  return { login, logout, me, bootstrap, authenticateByCookie, bootstrapRequired, cookieName };
}
