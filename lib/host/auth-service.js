...
export function createAuthService(store, cfg, ldapStrategy = null, oidcStrategy = null) {
  ...
  /**
   * 逃生通道：是否处于"无可用平台管理员"状态（已初始化但全部 system 用户被
   * 禁用/锁定/删除）。此时允许通过环回受限的 recovery 端点重建管理员。
   */
  function recoveryRequired() {
    return store.countUsers() > 0
      && !store.listUsers(null).some((u) => u.role === 'system' && u.status === 'active');
  }

  /**
   * 逃生重建平台管理员（仅当无可用管理员时；环回门禁在路由层）。
   * @returns {{ok: true, user: object, token: string} | {ok: false, error: {code, message}}}
   */
  function recovery({ username, password, ip = null, userAgent = null }) {
    if (!recoveryRequired()) {
      return { ok: false, error: { code: 'recovery-not-needed', message: 'active system admin exists' } };
    }
    const nameError = validateUsername(username);
    if (nameError) return { ok: false, error: { code: AUTH_ERRORS.INVALID_INPUT, message: nameError } };
    const passError = validatePasswordStrength(password);
    ...
    if (store.getUserByUsername(username)) {
      return { ok: false, error: { code: AUTH_ERRORS.CONFLICT, message: 'username already exists' } };
    }
    const id = store.createUser({ username, passwordHash: hashPassword(password), role: 'system' });
    const user = store.getUserById(id);
    store.writeAudit({ actorUserId: id, action: 'auth.recovery', result: 'success', ip });
    return issueSession(user, { method: 'recovery', ip, userAgent });
  }
  ...
  function me(cookieHeader) {
    ...
    if (auth) {
      return { ok: true, user: publicUser(auth.user), bootstrapRequired: false, recoveryRequired: false, methods: authMethods() };
    }
    if (bootstrapRequired()) {
      return { ok: true, user: null, bootstrapRequired: true, recoveryRequired: false, methods: authMethods() };
    }
    if (recoveryRequired()) {
      return { ok: true, user: null, bootstrapRequired: false, recoveryRequired: true, methods: authMethods() };
    }
    return { ok: false, error: { code: AUTH_ERRORS.UNAUTHENTICATED, message: 'not authenticated' } };
  }

  return { login, logout, me, bootstrap, recovery, recoveryRequired, authenticateByCookie, bootstrapRequired, authMethods, issueSession, cookieName };
}
