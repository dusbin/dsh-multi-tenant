 ...
 */
export function createAuthService(store, cfg, ldapStrategy = null) {
  ...
  function login({ username, password, method = null, ip = null, userAgent = null }) {
    ...
    const issueSession = (user) => {
      ...
      store.writeAudit({ actorUserId: user.id, tenantId: user.tenant_id, action: 'auth.login', detail: { method: method || 'auto' }, result: 'success', ip });
  ...
  return { login, logout, me, bootstrap, authenticateByCookie, bootstrapRequired, authMethods, cookieName };
}
