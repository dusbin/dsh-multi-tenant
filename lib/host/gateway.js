...
export function createGateway({ cfg, target, authService, quotaService, oidcStrategy, audit, logger = console }) {
  ...
  function handleUpgrade(req, socket, head) {
    ...
    // DSH 未升级（信任栅栏拒绝/状态异常，返回普通 HTTP 而非 101）：把响应转发给
    // 浏览器。此前此处无 'response' 监听 → 客户端 WS 请求永久挂起，且响应流
    // （upRes）无人消费，对端 RST 时 error 无监听可致 unhandled 崩溃。
    upstream.on('response', (upRes) => {
      responded = true;
      upRes.on('error', () => { try { socket.destroy(); } catch { /* ignore */ } });
      try {
    ...
    upstream.on('error', (error) => {
      ...
      if (!responded) rejectUpgrade(socket, 502, 'bad gateway');
...
function rejectUpgrade(socket, status, message) {
  // 浏览器可能在响应前断开（RST）——静默，避免 unhandled 'error' 崩溃
  socket.on('error', () => { /* client aborted */ });
