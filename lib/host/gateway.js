...
import { filterListValue } from './visibility.js';
import {
  AUTH_PREFIX,
  SECURITY_HEADERS,
  sendJson,
...
export function createGateway({ cfg, target, authService, quotaService, oidcStrategy, audit, logger = console }) {
  ...
  function proxyRaw(req, res, body) {
    ...
    const upstream = http.request(options, (upRes) => {
      res.writeHead(upRes.statusCode ?? 502, { ...upRes.headers, ...SECURITY_HEADERS });
      upRes.pipe(res);
    });
    track(upstream);
      ...
      upRes.on('end', () => {
        ...
        try {
          ...
          if (envelope && envelope.result && envelope.result.ok && envelope.result.value) {
            ...
            const respHeaders = { ...upRes.headers, ...SECURITY_HEADERS, 'content-length': String(Buffer.byteLength(payload)) };
