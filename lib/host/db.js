...
export function createStore(db) {
  return {
    ...
    insertTenantRaw(row) {
      db.prepare('INSERT INTO tenants (id, name, status, config, created_at) VALUES (?, ?, ?, ?, ?)')
        .run(row.id, row.name, row.status, row.config, row.created_at);
    },
    ...
    insertUserRaw(row) {
      db.prepare(
        `INSERT INTO users (id, tenant_id, username, email, password_hash, ldap_dn, oidc_sub, status, role, created_at, last_login_at, last_login_ip)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(row.id, row.tenant_id, row.username, row.email, row.password_hash, row.ldap_dn, row.oidc_sub, row.status, row.role, row.created_at, row.last_login_at, row.last_login_ip);
    },
    getUserById(id) {
    ...
    insertAuditRaw(row) {
      db.prepare(
        'INSERT INTO audit_logs (id, ts, actor_user_id, tenant_id, action, target_type, target_id, detail, result, ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ).run(row.id, row.ts, row.actor_user_id, row.tenant_id, row.action, row.target_type, row.target_id, row.detail, row.result, row.ip);
    },
    ...
    /** 重置系统：按外键安全顺序清空数据（keepUsage=true 时保留用量与配额累计）。 */
    wipeSystem({ keepUsage = false, useTransaction = true } = {}) {
      const run = () => {
        ...
        if (!keepUsage) {
          ...
          db.exec('DELETE FROM session_meter');
        }
    ...
    listAllUsageRecords() {
      return db.prepare('SELECT * FROM usage_records ORDER BY id').all();
    },
    insertUsageRecordRaw(row) {
      db.prepare(
        `INSERT INTO usage_records (id, ts, tenant_id, user_id, session_id, model, provider,
           input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, request_count)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(row.id, row.ts, row.tenant_id, row.user_id, row.session_id, row.model, row.provider, row.input_tokens, row.output_tokens, row.cache_read_tokens, row.cache_write_tokens, row.request_count);
    },
    listAllSessionMeter() {
      return db.prepare('SELECT * FROM session_meter ORDER BY session_id').all();
    },
    insertSessionMeterRaw(row) {
      db.prepare(
        `INSERT INTO session_meter (session_id, user_id, tenant_id, last_input, last_output, last_cr, last_cw, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(row.session_id, row.user_id, row.tenant_id, row.last_input, row.last_output, row.last_cr, row.last_cw, row.updated_at);
    },
    listAllAuthSessions() {
      return db.prepare('SELECT * FROM auth_sessions ORDER BY id').all();
    },
    listAllQuotaUsage() {
      return db.prepare('SELECT * FROM quota_usage ORDER BY scope, target_id, period, period_start').all();
    },
    insertQuotaUsageRaw(row) {
      db.prepare(
        'INSERT INTO quota_usage (scope, target_id, period, period_start, spent_tokens, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
      ).run(row.scope, row.target_id, row.period, row.period_start, row.spent_tokens, row.updated_at);
    },
    ...
    insertQuotaRaw(row) {
      db.prepare('INSERT INTO quotas (id, scope, target_id, token_limit, period, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(row.id, row.scope, row.target_id, row.token_limit, row.period, row.created_at, row.updated_at);
    },
