...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UsersSection() {
      ...
      var busyState = React.useState(false); var busy = busyState[0], setBusy = busyState[1];
      var msgState = React.useState(null); var msg = msgState[0], setMsg = msgState[1];
      var isSystemState = React.useState(false); var isSystem = isSystemState[0], setIsSystem = isSystemState[1];
      ...
      function load() {
        Promise.all([mt('user.list', {}), mt('me', {})])
          .then(function (r) {
            setList(r[0].users);
            setIsSystem(r[1].user.role === 'system');
            if (r[1].user.role === 'system') {
              return mt('tenant.list', {}).then(function (tv) {
                setTenants(tv.tenants || []);
                if (tv.tenants && tv.tenants.length && !tenantId) setTenantId(String(tv.tenants[0].id));
              });
            }
            setError(null);
            return null;
      ...
      function createUser() {
        setBusy(true); setMsg(null); setError(null);
        mt('user.create', { username: form.username, email: form.email || null, password: form.password, role: form.role || 'user', tenantId: isSystem ? Number(tenantId) : undefined })
