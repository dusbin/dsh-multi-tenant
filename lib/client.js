  ...
  factory: (require) => {
    ...
    var dict = {
      zh: {
        ...
        revokeSessions: '强制下线', revokedSessions: '该用户已强制下线', error_self_operation: '不能对自己执行该操作',
        // 通用
      ...
      en: {
        ...
        revokeSessions: 'Force logout', revokedSessions: 'User forced offline', error_self_operation: 'Cannot perform this on yourself',
    ...
    (function injectCss() {
      ...
      style.textContent = [
        ...
        /* 侧边栏用户芯片（设置旁） */
        '.mt-userchip{flex:1 1 auto;min-width:0;display:flex;align-items:center;gap:8px;padding:5px 8px;border-radius:10px;box-sizing:border-box;}',
        '.mt-userchip:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.05));}',
        '.mt-userchip-avatar{flex:none;width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#2563eb,#7c3aed);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;letter-spacing:.02em;}',
        '.mt-userchip-meta{flex:1;min-width:0;display:flex;flex-direction:column;line-height:1.3;}',
        '.mt-userchip-name{font-size:12px;color:var(--dsw-alias-label-primary,#e8eaf0);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}',
        '.mt-userchip-sub{font-size:10px;color:var(--dsw-alias-label-tertiary,#6b7280);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}',
        '.mt-userchip-logout{flex:none;display:inline-flex;align-items:center;gap:4px;height:26px;padding:0 9px;border:1px solid var(--dsw-alias-border,#2a3242);border-radius:7px;background:transparent;color:var(--dsw-alias-label-secondary,#9aa3b2);font-size:12px;font-family:inherit;cursor:pointer;}',
        '.mt-userchip-logout:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.06));color:var(--dsw-alias-label-primary,#e8eaf0);}',
        '.mt-userchip-logout:disabled{opacity:.5;cursor:default;}',
        '.mt-userchip-logout.rail{width:36px;height:36px;border:none;border-radius:50%;justify-content:center;padding:0;color:var(--dsw-alias-label-tertiary,#6b7280);}',
        '.mt-userchip-logout.rail:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.06));color:var(--dsw-alias-label-secondary,#9aa3b2);}',
      ].join('');
    ...
    // ---------------------------------------------------------------------
    // 侧边栏用户芯片（设置旁）：当前用户名 + 退出登录
    // 注入 sidebar.footer.action（设置按钮正上方的 footer 动作区，与 Cordis
    // 徽章共存）。宽态：头像 + 用户名 + 退出按钮；收起态：圆形退出图标按钮。
    // ---------------------------------------------------------------------
    function logoutIconSvg() {
      return e('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2.2, strokeLinecap: 'round', strokeLinejoin: 'round' },
    ...
    function UserChip(props) {
      ...
      React.useEffect(function () {
        var cancelled = false;
        mt('me').then(function (v) {
          if (!cancelled && v && v.user) setMe(v);
        }).catch(function () { /* 未登录/会话失效：由登录门禁接管 */ });
        return function () { cancelled = true; };
      }, []);
      ...
      var name = me.user.username || '';
      var sub = roleLabel(me.user.role) + ((me.tenant && me.tenant.name) ? ' · ' + me.tenant.name : '');
      var initials = name ? name.slice(0, 2).toUpperCase() : '?';

      if (!wide) {
        return e('button', {
          className: 'mt-userchip-logout rail',
          onClick: logout,
          disabled: busy,
          title: (name ? name + ' · ' : '') + t('logout'),
          'aria-label': t('logout'),
        }, logoutIconSvg());
      }
      return e('div', { className: 'mt-userchip' },
        e('div', { className: 'mt-userchip-avatar' }, initials),
        e('div', { className: 'mt-userchip-meta' },
          e('div', { className: 'mt-userchip-name', title: name }, name),
          e('div', { className: 'mt-userchip-sub' }, sub),
        ),
        e('button', { className: 'mt-userchip-logout', onClick: logout, disabled: busy, title: t('logout') },
          logoutIconSvg(), t('logout')),
      );
    }
    ...
    function UsersSection() {
      ...
      var meState = React.useState(null); var me = meState[0], setMe = meState[1];
      ...
      function load() {
        ...
        Promise.all([
          ...
          mt('me', {}).catch(function () { return { user: null }; }),
        ]).then(function (r) {
          setList(r[0].users);
          ...
          e('tbody', null, (list || []).map(function (u) {
            return e('tr', { key: u.id },
              ...
              e('td', null, e('div', { className: 'mt-row' },
                me && me.id === u.id
                  ? e('span', { className: 'mt-hint', style: { textAlign: 'left', marginTop: 0 } }, t('me'))
                  : e(React.Fragment, null,
                    u.status === 'active'
    ...
    exports.apply = function apply(pluginCtx, config) {
      ...
      try {
        ...
        ctx.slots.inject('sidebar.footer.action', function () {
          var dispose = ctx.slots.register(
            { name: 'sidebar.footer.action', id: 'dsh-multi-tenant-user-chip', order: 200 },
            UserChip,
          );
          return function () { dispose(); };
        });
